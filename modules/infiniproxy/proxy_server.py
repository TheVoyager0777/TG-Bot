...
def resolve_backend_and_model(
    ...
    if not backend:
        ...
        if not backend:
            ...
            }, config.openai_model, openai_client  # always use configured model (ignore Claude model name)
...
async def create_message(
    ...
    try:
        ...
        # Streaming is NOT translated to Anthropic SSE — always force non-streaming.
        # The translator already sets stream=false; do NOT override it.
        ...
        # Always use non-streaming path — streaming SSE translation is not implemented.
        # The translator already forces stream=false on the upstream request.
        if is_streaming_requested:
            logger.info("SDK requested streaming → responding with non-streaming (proxy translates full response)")

        # Non-streaming path (used for all requests)
