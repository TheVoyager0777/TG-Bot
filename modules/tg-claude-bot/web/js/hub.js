...
function hubRenderTakeover(sessions, loading) {
  ...
  try {
    sessions.forEach(s => {
      ...
      html += '<div class="cc-item" data-id="' + esc(sid) + '" data-project="' + esc(s.project || '') + '">';
      html += '<div class="cc-title">' + esc(title) + '</div>';
      if (lastMsg) {
        html += '<div class="cc-last">' + esc(roleIcon) + ' ' + esc(lastMsg.slice(0, 80)) + (lastMsg.length > 80 ? '…' : '') + '</div>';
      }
      html += '<div class="cc-meta">' + esc(s.project || '') + ' · ' + esc(age) + ' · ' + esc(sid.slice(0, 8)) + '…</div>';
      html += '<button class="cc-resume-btn" data-id="' + esc(sid) + '" data-project="' + esc(s.project || '') + '">接管</button>';
      html += '</div>';
    });
  } catch (e) {
    ...
    btn.addEventListener('click', async () => {
      ...
      try {
        ...
        const r = await fetch(rurl, {
          ...
          body: JSON.stringify({ session_id: sid, project: project })
        });
        const j = await r.json();
        if (j.ok) {
          ...
          try { hubRefreshActive(); } catch (e) {}   // 立刻刷新已接管列表
        } else {
