"""Version metadata for phantom-network."""
...
__version__ = "0.1.0"
DESCRIPTION = "Phantom network contact book and Cloudflare domain pool"
...
def _scan_sources() -> tuple[float, int]:
    ...
    for root, dirs, files in os.walk(_HERE):
        ...
        for name in files:
            if not (name.endswith(".py") or name.endswith(".sh") or name.endswith(".md")):
                continue
            count += 1
            try:
