"""Version metadata for phantom-console."""
...
def _scan_sources() -> tuple[float, int]:
    ...
    for root, dirs, files in os.walk(_HERE):
        ...
        for name in files:
            if not (name.endswith(".py") or name.endswith(".js") or name.endswith(".css")
                    or name.endswith(".html") or name.endswith(".webmanifest")):
                continue
            count += 1
            try:
