...
function _renderSessionTabsNow(sessions) {
  ...
  if (session && !names.includes(session)) {
    session = null;
    try { localStorage.setItem('phantom_session', '__all__'); } catch (e) {}
  ...
  html += names.map(n => {
    ...
    const hist = info.historyOnly ? ' history' : '';
    const badge = info.queued ? '<span class="badge">' + info.queued + '</span>' : (info.historyOnly ? '<span class="badge">H</span>' : '');
    return '<button class="stab' + active + busy + hist + '" data-session="' + esc(n) + '">' +
      esc(n) + badge + '</button>';
  }).join('');
