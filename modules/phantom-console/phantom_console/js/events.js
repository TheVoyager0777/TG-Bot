...
function handle(ev) {
  ...
  switch (ev.type) {
    ...
    case 'assistant':
      s.hasBody = true;
      renderText(s, { ...ev, type: 'text' });
      break;

    case 'subagent_text':
      s.hasBody = true;
      renderSubagentText(s, ev);
      break;

    case 'tool':
      // TodoWrite 有两条产线：LiveMessage 旁路成独立 'todo' 事件（不发 tool 事件）；
      // 外部/原始产线发 tool 事件且带 input.todos。后者在此就地转待办，避免漏更新。
      if (!ev.parent && (ev.tool === 'TodoWrite' || (ev.input && ev.input.todos))) {
    ...
    case 'thinking':
      s.hasBody = true;
      if (ev.parent) renderSubagentThinking(s, ev);
      else renderThinking(s, ev);
      break;
